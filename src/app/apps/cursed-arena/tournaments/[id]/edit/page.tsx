...
import { fetchTournament, updateTournament } from '@/lib/cursed-arena/api';
...
export default function EditTournamentPage() {
  ...
  useEffect(() => {
    fetchTournament(id).then(setData).catch(console.error).finally(() => setLoading(false));
  }, [id]);

  const handleSubmit = async (values: any) => {
    await updateTournament(id, values);
    router.push(`/apps/cursed-arena/tournaments/${id}`);
  };
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center" style={{ backgroundColor: '#0a0a0f' }}>
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-2" style={{ color: '#ffffff' }}>Tournament Not Found</h2>
          <button
            onClick={() => router.push('/apps/cursed-arena/tournaments')}
    ...
    <div className="min-h-screen w-full" style={{ backgroundColor: '#0a0a0f' }}>
      <div className="w-full px-6 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold mb-3" style={{ color: '#ffffff' }}>Edit Tournament</h1>
          <p className="text-lg" style={{ color: '#808090' }}>Update {data.name}</p>
        </div>

        <EntityForm entityDef={tournament} initialValues={data} onSubmit={handleSubmit} mode="edit" />
      </div>
    </div>
  );
}
